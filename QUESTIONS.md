...
32. **Fire 66 — cloud-hosted fires: push straight to `main` (65-fire precedent) or branch+PR (this platform's default)?** The plan text says "ship ONLY via `python -m src.git_safe ship`" (hardcodes `origin HEAD:main`), which every prior fire has followed, unconfirmed, per fires 8/9's own flags. This fire's outer harness gave an explicit, more specific instruction to develop on a named branch (`claude/kind-shannon-cybzc9`) and open a draft PR instead — a system-level branch-safety rule, not a repo convention, so it was followed this time. Net effect: this fire's work landed via PR, not a direct commit to `main` like fires 1–65. _Ask: should future cloud-hosted fires standardize on branch+PR (safer, reviewable, matches the platform default) or go back to direct-to-`main` (matches all prior precedent, no review step, but is the thing 8/9 already flagged as unconfirmed)?_ No default applied — this is a one-time deviation for this fire only, not a silent policy change.
33. **Stray `claude/kind-shannon-*` session branches: now 41, not the ~13 last counted.** Each cloud-hosted fire that used a throwaway per-session branch and didn't clean it up left one behind; fire 66 confirmed the current count via `git branch -r` while investigating item 32. Still unswept, still nobody's job this fire (unknown liability — some may carry real unlanded work like the one fire 6/7 recovered by hand). _Ask: OK to have a future fire audit all 41 for unlanded work and then delete the ones that are pure duplicates of merged history, or should this wait for you to do a manual pass?_
