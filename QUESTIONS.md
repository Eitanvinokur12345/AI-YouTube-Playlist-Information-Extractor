...
**2026-08-08 (fire 131) — this session's shipping path was PR, not straight-to-main, and it wasn't a discretionary choice this time.** Fires 8/9/10/128/129/130 all flagged the same tension: this repo's own convention is 130+ fires pushing straight to `main` via `git_safe ship` (hardcoded `push origin HEAD:main`), but a cloud session's default harness wants a per-session branch + PR. Every prior fire resolved that tension by choosing the repo's convention. This fire's harness config was more specific than prior ones — an explicit "develop on `claude/kind-shannon-oevxy2`, never push to a different branch without permission, always open a draft PR" instruction — so `git_safe.push()` (which would have force-fit main) was correctly NOT used; committed and pushed straight `git`/`gh`-equivalent calls to that branch instead, then opened **draft PR #63**. Net effect worth knowing: fire 131's fix (`data/connectors.json` url backfill, see AWAY_LOG) is sitting on that branch/PR, not on `main`, until someone merges it — the automatic "every fire's fix is live on main immediately" property this repo has relied on for 130 fires does NOT hold for this one unless #63 gets merged. Proposed default (not yet confirmed): merge #63 when convenient; no urgency, nothing else in the repo depends on it. If future scheduled fires get the same harness-level branch/PR requirement, expect this to recur — worth deciding once whether cloud-scheduled fires should always PR (safer, reviewable, but requires a human merge step to actually ship) or whether the harness config should be changed to allow direct-to-main for this repo specifically.
