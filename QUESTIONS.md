...
**2026-07-29 (fire 66) — a 4th, slightly different data point on the still-unconfirmed branch-vs-main
push question (see fires 8/9/10 above).** Every prior fire (8 through 65) pushed straight to `main` via
`src/git_safe.py`'s hardcoded `push origin HEAD:main`, treating branch+PR as avoidable overhead the repo's
own convention had already settled. This fire's container came up under a scheduled-session harness whose
integration rules are not just a *default* toward branch+PR — they're an explicit contract: develop only on
the named branch (`claude/kind-shannon-vnhxjk`), never push elsewhere without explicit permission. That is
a harder constraint than anything previous cloud fires worked under, so I did not override it to match
`git_safe`'s main-push convention — I fast-forward-merged the one trailing `origin/main` commit into the
branch (clean, no divergence, confirmed via `merge-base`), shipped fire 66's small housekeeping change there,
pushed the branch, and opened **PR #34** against `main` rather than committing to `main` directly. _Ask: for
scheduled/unattended sessions specifically (as opposed to the GitHub Actions cron beat, which has direct
push access and isn't affected by this), do you want them to (a) keep opening small PRs like #34 that you
merge at your convenience, or (b) is there a way to grant that harness direct-push permission so it can match
the other 57+ fires' convention?_ Default while unanswered: keep opening PRs for this harness class of
session — it's the reversible, ask-first-adjacent choice, and merging #34 (or any future one like it) takes
one click whenever you're back.
