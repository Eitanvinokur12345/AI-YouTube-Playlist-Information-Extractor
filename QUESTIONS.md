...
**2026-07-26 (cloud scheduled fire) — this fire ran in a different environment than the away-mode contract assumes, and the branch-fragmentation problem is worse than fire 7 estimated.**
This fire came from a *cloud-hosted* scheduled Claude Code session (claude.ai/code), not the local `D:\AI-YouTube-Skills` PC loop the away-mode contract (`data/excava/away_mode.json`) and `git_safe.py`'s permission notes were written for. Two consequences: (1) this environment enforces a branch+draft-PR workflow rather than direct pushes to `main`, so this fire's own work lands on yet another throwaway `claude/kind-shannon-*` branch instead of `main` — the exact failure mode fires 6/7 flagged — unless a PR is opened immediately (done this fire, see the PR for this branch). (2) A full remote branch sweep found **15 stray `claude/kind-shannon-*` branches** on origin (not ~13–14 as fire 7 estimated), and this fire's own branch (`ym7kbd`) happened to sit at the exact tip of `origin/main` (0 ahead/behind) — pure luck of when it was cloned, not something to rely on. Diffing one stray branch (`ae4swi`) against `main` showed **331 files changed, ~77k/~76k line insertions/deletions** — the histories are essentially fully-independent rewritten trees, not a few commits apart, so a real reconciliation needs per-file content review, not a mechanical merge/cherry-pick. → **Proposed default (did NOT attempt): do nothing destructive with the other 14 stray branches this fire** — a full sweep is too large and too risky to do unattended/unverified; flagging for your return rather than guessing at which of ~77k changed lines are real improvements vs. duplicate autonomous churn. **Also proposed:** treat cloud-scheduled fires as informational/audit-only (standing checks, docs, QUESTIONS.md/AWAY_LOG.md updates) rather than code/data-mutating "away loop increments," since their git workflow doesn't match the direct-to-main convention the rest of this contract assumes — until that's reconciled (e.g. the cloud schedule's PR gets fast-tracked and merged rather than left open like another stray branch).
