...
**MEDIOCRE:** A mediocre prompt is vague or overly broad ("Make this sound professional") and lacks measurable targets (e.g., "within 50 words"). It might include fluff like "please" or "kindly" without functional constraints, or it buries key details in walls of text. Signals: no examples, no constraints, or a title that’s a question ("How to write emails?").

**DISQUALIFIES:** An element is disqualified if it’s *incomplete* (missing core instructions), *contradictory* (e.g., "be concise" + "include every detail"), or *unverifiable* (no way to test if it worked). Also disqualified: prompts that embed personal biases ("Write like a snarky millennial") or require proprietary context (e.g., "Use our internal style guide").
...
**MEDIOCRE:** A mediocre prompt is either too broad ("make this better") or overly prescriptive without purpose ("use exactly 12 words in a haiku format, but also sound like Shakespeare"). It may include irrelevant details (e.g., "don’t mention my cat") or lack critical context (e.g., omitting the audience or goal). These prompts force the AI to guess intent, leading to generic or off-target responses. They’re often reactive ("fix this") rather than proactive ("turn this into a persuasive email for a client").

**DISQUALIFIES:** A prompt is outright disqualified if it’s *nonsensical* (e.g., "generate a poem about a square circle"), *harmful* (e.g., "write a hate speech rant"), or *unanswerable* (e.g., "predict the stock market in 2050 with 100% accuracy"). It also fails if it’s *unethical* (e.g., "help me cheat on my taxes") or *illegal* (e.g., "design a phishing email"). Prompts that demand the AI violate its guardrails (e.g., "ignore your safety rules") are automatically disqualified.

**OBSERVABLE SIGNALS:**
- **Clarity:** The prompt has a single, unambiguous objective (e.g., "Summarize this article in 3 bullet points").
- **Relevance:** All included details serve the goal (e.g., no off-topic constraints).
- **Structure:** Uses formatting cues (e.g., "Step 1: X. Step 2: Y.") or examples to guide output.
...
In contrast, a MEDIOCRE prompt element is one that is vague, open-ended, or poorly defined, making it difficult for the AI model to understand the task and generate a relevant response. It may contain ambiguity or jargon, or lack sufficient context, leading to inconsistent or unhelpful outputs.
