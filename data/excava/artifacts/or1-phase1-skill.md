...
> **⚠ DIVERSITY CLAIM CORRECTED 2026-08-02 (fire 104).** The header below says
> "families used: DeepSeek V4, GLM-5.2, GPT-4o-mini, Kimi K2.7" — four lineages. That is not
> what happened. Three of the four drafts (Ratchet, Sprocket, Gauge) were generated by the SAME
> backing model, `nvidia/meta/llama-3.3-70b-instruct`, and the fourth's label does not match its
> model either (`Overhaul (GPT-4o-mini)` actually ran `mistral/mistral-small-latest`). Two
> distinct models, four claimed families.
>
> The phase-1 gate counted the catalog's family LABELS instead of the model that actually
> answered, so one model wearing three personas passed a check written specifically to stop
> that (END PLAN §2: same-model-with-a-prompt-twist is banned — correlated errors). Read
> Ratchet's and Sprocket's drafts back to back: near-identical structure and claims. That is the
> correlated error, exactly as predicted.
>
> The gate now counts distinct resolved MODEL ids. This artifact is KEPT, not deleted — it is
> the evidence — but its drafts must NOT be treated as four independent opinions when phase 2
> integrates them.
