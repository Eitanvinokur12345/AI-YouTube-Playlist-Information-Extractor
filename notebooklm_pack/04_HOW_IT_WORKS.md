# EXCAVA — how the system actually works
...
# ===== FILE: ANALYZE_SPEC.md =====

# ANALYZE_SPEC — the YouTube→hub analysis stage (project spec)

> **Moved from `CLAUDE.md` on 2026-07-30, at Eitan's request.** `CLAUDE.md` is now reserved for
> general, cross-project Claude guidance so it stays useful on his other projects; everything
> project-specific lives here. This file is the **authoritative spec** for the analysis stage and
> is referenced by `.github/workflows/analyze.yml`, `discover.yml`, and by rule-citations in
> `src/*.py` (the older ones still say "CLAUDE.md Step N" — same steps, this file).
