...
def or1_phase1(element_type: str, min_families: int = 2, roles=("doer", "checker", "improver", "lead")) -> dict:
    ...
    # DIVERSITY IS DECIDED BY THE MODEL THAT ANSWERED, NOT BY THE LABEL WE ASKED FOR.
    # Fire 104 caught this gate passing on a fake: or1-phase1-skill.md declared
    # "families used: DeepSeek V4, GLM-5.2, GPT-4o-mini, Kimi K2.7" while three of the four
    # drafts had actually been generated by the SAME backing model
    # (nvidia/meta/llama-3.3-70b-instruct), and the fourth's label did not match its model
    # either. The catalog's family labels are a menu; the resolved `model` is the meal. Counting
    # labels made a one-model run look like a four-lineage debate — precisely the
    # same-model-with-a-prompt-twist §2 bans, waved through by the very check built to stop it.
    # The two drafts from the shared model came back near-identical, which is the correlated
    # error the whole 4-phase process exists to avoid.
    models = sorted({(d.get("model") or "").strip() for d in ok if (d.get("model") or "").strip()})
    labels = sorted({d["family"] for d in ok})
    mismatch = len(models) < len(labels)
    return {"ok": len(models) >= min_families,
            "element_type": element_type, "phase": 1, "drafts": drafts,
            "families_used": labels,
            "models_used": models,
            "distinct_models": len(models),
            "label_vs_model_mismatch": mismatch,
            "reason": (None if len(models) >= min_families else
                       f"{len(labels)} family LABEL(s) but only {len(models)} distinct MODEL(s) "
                       f"actually answered ({', '.join(models) or 'none'}) — that is one model "
                       "wearing personas, which is what §2 bans. Phase 1 refuses rather than "
                       "produce drafts whose errors are correlated by construction.")}
