...
answers) got copy-pasted into three call sites since (verify_elements.py fire 50,
verify_connectors.py fire 50, github_meta_enrich.py fire 51) instead of shared — this module
is the shared version so a fourth caller (standing_checks.py, fire 124) doesn't need a fourth
copy, and so the next fire that wants to KNOW the verdict up front (before choosing which
increment to attempt) has one place to ask instead of re-deriving it by hand.
...
# Lanes that self-abort (by design, via their own copy of this same canary) when egress is
# restricted — kept here so a caller can explain the consequence, not just the verdict.
