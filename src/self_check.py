...
def CHECKS():
    ...
    return [
     ...
     (10, "Specific model version + correct routing", lambda c: _frac_ok(c["skills"], lambda s: bool(s.get("model_version")), 0.3)),
     ...
     (20, "No duplicate model entries", lambda c: (_dups(c["models"], lambda m: (m.get("name","")+m.get("model_version","")).lower()) == 0, "unique")),
